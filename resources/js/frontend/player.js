    ...
    if (source_type == 'youtube') {
        ...
        if(file_type == 'doc'){
            ...
            let player = `<div class="file_type">
                            ...
                            <h6 class="mt-3">Type: File</h6>
                            ${source_type == 'external_link' ? `<a href="${source}" target="_blank" class="common_btn mt-3">Download</a></div>` : `<a href="${base_url}/student/file-download/${id}" class="common_btn mt-3">Download</a></div>` }
                          </div>`;
...
$('.lesson').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.about_lecture').html(data.description);
